...
from articles.models import AnalyzedArticle, StockItem
...
class Command(BaseCommand):
    help = (
        ...
        '(RSS 키워드 수집(collect_keyword_news)과 병행되는 별도 소스).'
    )
    ...
    def handle(self, *args, **options):
        ...
        for row in rows:
            ...
            self.stdout.write(self.style.SUCCESS(
                f"    ↳ [KIS 뉴스/공시 매칭] {matched_stock.name} ➔ {row['title'][:30]}..."
            ))
            ...
            AnalyzedArticle.objects.create(
                ...
                ai_analysis=f"KIS 종합 시황_공시(제목) API로 수집된 {matched_stock.name} 관련 뉴스/공시입니다.",
                blog_content=row['title'],
                applied_template='T1',
                is_premium=False,
                is_posted=False,
            )
