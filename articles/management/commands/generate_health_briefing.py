...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # UTC 날짜를 준다 — 오전(KST 08:00 = UTC 전날 23:00)처럼 KST 자정을 걸치는 시간대에
        # 돌리면 하루 어긋난 날짜로 저장/조회될 수 있어, 여기서는 KST로 직접 변환해서 쓴다.
        today = timezone.now().astimezone(KST).date()

        pseudo_url = f"internal://health-briefing/{today}/{session}"
        ...
        draft = article_ai.generate_health_article(
        ...
        self.stdout.write(self.style.SUCCESS(f"🎉 {session_label} 건강정보 저장 완료: {info['title']}"))
