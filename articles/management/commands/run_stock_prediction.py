...
# BUY/SELL 발동에 필요한 최소 확률. 2026-08-08 재측정(확정된 실제 결과가 있는 라이브 예측
# 12,199건을 신뢰도 구간별로 누적 집계): confidence(=|prob-0.5|*2) 0.2 구간(=이 값 0.60)은 실현
# 정확도 57.4%(표본 5,394건)였는데, 0.5 구간(=이 값 0.75, 표본 637건)은 63.1%까지 올라갔다.
# 그 위 0.6 구간(=이 값 0.80, 표본 181건)은 표본이 충분한데도 59.7%로 오히려 꺾여 0.75가
# 최적점이고, 0.7 구간 이상(표본 41건 이하)은 표본이 너무 적어 채택하지 않았다. 임계값을
# 올리면 신호가 뜨는 종목 수는 줄어드는 대신(정확도-커버리지 트레이드오프) 신호 자체의 정확도는
# 올라간다 — 단, 이 수치는 표본이 쌓일수록 계속 변하니(최초 측정 2026-08-07 때는 같은 0.75
# 구간이 표본 374건으로 69.3%였다) 주기적으로 재집계가 필요하다.
