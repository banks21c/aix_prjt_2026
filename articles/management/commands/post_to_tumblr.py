...
class Command(BaseCommand):
    help = (
        ...
        '마이페이지에서 텀블러(Tumblr) 자동 포스팅을 연동/활성화한 회원들의 각자 블로그에 발행합니다.'
    )
    ...
    def handle(self, *args, **options):
        ...
        accounts = enabled_accounts('TUMBLR', category=options.get('category'))
        if not accounts.exists():
            self.stdout.write(self.style.WARNING("[-] 텀블러 자동 포스팅을 연동/활성화한 회원이 없습니다."))
            return
        ...
        for account in accounts:
            ...
            self.stdout.write(self.style.SUCCESS(
                f"🚀 {account.user.username} ({account.site_url}) 텀블러 발행을 시작합니다. 대상: {len(candidates)}건"
            ))
            ...
            for article in candidates:
                ...
                if ok:
                    self.stdout.write(self.style.SUCCESS(
                        f"    ↳ [발행 성공] {article.title[:40]} 글이 {account.user.username}의 텀블러에 등록되었습니다: {result or '(비공개 초안)'}"
                    ))
                else:
                    self.stdout.write(self.style.ERROR(f"    ↳ [{account.user.username}] {result}"))

        self.stdout.write(self.style.SUCCESS("🎉 오늘의 모든 회원 텀블러 발행 프로세스가 종료되었습니다!"))
